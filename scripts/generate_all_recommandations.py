...
def main():
    ...
    # 2. Charger le modèle ALS entraîné
    model = ALSModel.load("hdfs://namenode:9000/movielens/models/als_best")

    # 3. Récupérer tous les userId de la base
    db = get_db()
    ...
    # 4. Construire un DataFrame Spark des utilisateurs
    users_df = spark.createDataFrame(
        [(int(uid),) for uid in user_ids],
        schema=StructType([StructField("userId", IntegerType(), False)])
    )

    # 5. Générer top-5 recommandations pour chaque utilisateur
